function response = svr(Setslog, svstr, trend)
...
%     in = Setslog.dataset(k).in;
    ...
    for l = 1:noinnodes
        ...
        if k == Setslog.no_run
            hold off
            plot(1:length(Prey(:,1)), Prey(:,l), 'g','linewidth',2); hold on
        ...
        if k == Setslog.no_run
            text(270, 0.5, s, 'fontsize', 18)
            s = strcat('svr-', svstr, '-', num2str(k), '-', num2str(in_index(l)))
            %         title(gca, s,'fontsize',18);
            saveas(gcf, char(s), 'jpg');
            %pause(0.5)
        end
    end
